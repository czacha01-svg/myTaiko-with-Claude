...
void updateDisplay() {
  ...
  if (menuIndex == 5) display.print(mode == 1 ? "Switch" : "PC");
  else display.print(*menuValues[menuIndex]);
  display.setTextSize(1);
...
void handleEncoder() {
  ...
  if (currentButton == LOW && lastButtonState == HIGH) buttonHoldStart = millis();
...
void handleExpanderButtons() {
  bool states[8];
  for (int i = 0; i < 8; i++) states[i] = (expander.digitalRead(i) == LOW);
...
void setup() {
  ...
  expander.begin();
  for (int i = 0; i < 8; i++) expander.pinMode(i, INPUT_PULLUP);
